...
%% Non-local means filter using integral image:
%% Initialization:
...
for p=-Ds:Ds
    for q=-Ds:Ds       
        %Skip the current calculation when (p,q) equals (i,j) in (1-6):
        if(p==0&&q==0)
            continue;
        end
        ...
        % Calculate the Euclidean distance using (1-6) or (1-7):
        ...
        S=S./(d^2); 
        % Calculate the weight in (1-3):
        weight=exp(-S./(h*h)+10e-60);
...
N_f=N_f+maxweight;
% Using (1-3) to calculate the final result:
